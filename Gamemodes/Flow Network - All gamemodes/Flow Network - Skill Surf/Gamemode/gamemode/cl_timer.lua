...
function Timer:SetLegitSpeed( nTop )
	if lp and IsValid( lp() ) then
		...
		Link:Print( "Surf Timer", "Your maximum velocity has been changed to " .. nTop )
	end
end
...
function Timer:SetCheckpoint( nID, bClear, szSpeed )
	if bClear then
		...
		Link:Print( "Surf Timer", "Checkpoint #" .. nID .. " has been cleared!" )
	else
		Cache.C_Data[ nID ] = os.date("%H:%M:%S", os.time()) .. " (" .. szSpeed .. ")"
		Link:Print( "Surf Timer", "Checkpoint saved to #" .. nID )
	end
...
function GM:HUDPaintBackground()
	...
	if lpc:Team() == ts then
		...
		if IsValid( ob ) and ob:IsPlayer() then
			...
			local cp = cl( nSpeed - 500, 0, 5000 ) / 5000
