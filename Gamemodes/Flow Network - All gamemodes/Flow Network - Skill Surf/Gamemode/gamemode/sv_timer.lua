...
function PLAYER:StartTimer()
	...
	if vel2d > CAPVEL and not (bit.band( Timer.Options, Zones.Options.NoStartLimit ) > 0) then
		...
		return Core:Send( self, "Print", { "Surf Timer", Lang:Get( "ZoneSpeed", { math.ceil( vel2d ) .. " u/s" } ) } )
	end
...
function PLAYER:RestartPlayer()
	Command:RemoveLimit( self )
	Command.Restart( self )
	Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "ZoneRestart" ) } )
end
...
function Timer:Finish( ply, nTime )
	...
	Core:Send( ply, "Print", { "Surf Timer", Lang:Get( szMessage, { Timer:Convert( nTime ), szSlower, szSync } ) } )
	
	local OldRecord = ply.Record or 0
	if ply.Record != 0 and nTime >= ply.Record then return end
	
	ply.Record = nTime
	ply.SpeedRequest = { ply.Style, OldRecord }
...
function Timer:LoadRecords()
	...
	local Map = sql.Query( "SELECT nMultiplier, nTier, nType, nBonusMultiplier, nOptions FROM game_map WHERE szMap = '" .. game.GetMap() .. "'" )
	if Core:Assert( Map, "nMultiplier" ) then
		...
		Timer.Tier = tonumber( Core:Null( Map[ 1 ]["nTier"], 1 ) )
		Timer.Type = tonumber( Core:Null( Map[ 1 ]["nType"], 0 ) )
...
function Timer:AddRecord( ply, nTime, nOld )
	...
	Core:Send( ply, "Print", { "Surf Timer", Lang:Get( szID, Data ) } )
	
	Data[ 2 ] = ply:Name()
	Core:Broadcast( "Print", { "Surf Timer", Lang:Get( szID, Data ) }, ply )
...
function Timer:AddSpeedData( ply, tab )
	if ply.Record and ply.Record > 0 and ply.SpeedRequest then
		local szData = Core.Util:TabToString( { math.floor( tab[ 1 ] ), math.floor( tab[ 2 ] ), ply.SpeedRequest[ 2 ], ply.LastSync or 0 } )
		sql.Query( "UPDATE game_times SET vData = '" .. szData .. "' WHERE szUID = '" .. ply:SteamID() .. "' AND szMap = '" .. game.GetMap() .. "' AND nStyle = " .. ply.SpeedRequest[ 1 ] )
...
function Timer:CPHandleCallback( ply, nID, bDelay, bDelete, bForceSave )
	...
	if nID == 1 then
		if bDelete then
			return Core:Send( ply, "Print", { "Surf Timer", "You can't delete the most recent checkpoint like this." } )
		end
		...
		if #ply.Checkpoints > 0 then
			...
			Core:Send( ply, "Print", { "Surf Timer", "No checkpoints found. Set one first!" } )
		end
	else
