...
function Spectator:Clear()
	...
	Spectator.Mode = 3
	Timer:SetSpecData( {}, true, 0, true )
end

function Spectator:RemoteSpec( list )
	Spectator.RemList = list
	Timer:SetSpecData( list, true, #list )
end
...
function Spectator:Viewer( data )
	...
	local nCount = 0
	for _, spec in pairs( Spectator.OwnList ) do
		nCount = nCount + 1
	end
	Timer:SetSpecData( Spectator.OwnList, false, nCount )
end
...
function Spectator:Timer( data )
	if not data[1] then
		...
		Spectator.Data.Start = data[2] and tonumber(data[2]) + Timer:GetDifference() or nil
