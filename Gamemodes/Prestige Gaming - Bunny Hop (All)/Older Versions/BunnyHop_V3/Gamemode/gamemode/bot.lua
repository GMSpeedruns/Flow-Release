		...
		if v:IsBot() then
			...
			if Bot.Data then
				timer.Simple(5, function()
					...
					v:SetNWInt("BhopRec", tonumber(Bot.Data["Time"]))
...
function Bot.Leader:Qualified(ply)
	...
	if ply.CurrentRecord and ply.CurrentRecord > 0 then
		if #Records.List[MODE_NORMAL] > 0 then
			if ply.CurrentRecord <= Records.List[MODE_NORMAL][#Records.List[MODE_NORMAL]][2] then return true end
		end
		if #Records.List[MODE_AUTO] > 0 then
			if ply.CurrentRecord <= Records.List[MODE_AUTO][#Records.List[MODE_AUTO]][2] then return true end
		end
	end
	return false
end
