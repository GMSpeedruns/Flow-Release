...
function RTV:LoadData()
	...
	local Unique = sql.Query( "SELECT szMap, nTier, nType, nMultiplier FROM game_map" )
	if Core:Assert( Unique, "szMap" ) then
		for _,data in pairs( Unique ) do
			table.insert( MapList, { data["szMap"], Core:Null( data["nMultiplier"], 1 ), Core:Null( data["nTier"], 1 ), Core:Null( data["nType"] ) } )
		end
	end
