...
function PLAYER:FinishMap(time)
	...
	if self.CurrentRecord == 0 or time < self.CurrentRecord then
		...
		if OldTimes == NewTimes then
			...
			if OldRecord == 0 then
				SendGlobalMessage(MSG_WR_NOR_FINISH, {self:Name(), ConvertTime(time), MODE_NAME[self.HopMode]}, self)
			else
				SendGlobalMessage(MSG_WR_IMPR_FINISH, {self:Name(), ConvertTime(OldRecord - time), ConvertTime(time), MODE_NAME[self.HopMode]}, self)
			end
		else
			...
			if OldRecord == 0 then
				SendGlobalMessage(MSG_WR_TOP_FINISH, {self:Name(), rank, ConvertTime(time), MODE_NAME[self.HopMode]}, self)
			else
				SendGlobalMessage(MSG_WR_TOPIMPR_FINISH, {self:Name(), rank, ConvertTime(time), ConvertTime(OldRecord - time), MODE_NAME[self.HopMode]}, self)
			end
		end
	end
end
...
local function RemoveVote(ply)
	SpectateEnd(ply)
	SendGlobalMessage(MSG_BHOP_LEFT, {ply:Name()}, ply)
...
hook.Add("PlayerDisconnected", "RemoveVote", "RemoveVote")
