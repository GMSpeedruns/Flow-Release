...
function ExecMapChecks(map, ply)
	if ply and ply:IsValid() then
		...
		if MAP_BONUS and ply.HopMode == MODE_BONUS then
			ply:SetPos(MAP_BONUS + Vector(0, 0, 64))
		end
		return
	end
...
function CreateAreaBoxes()
	...
	SQLQuery("SELECT szData, nType FROM bhop_mapareas WHERE szMap = '" .. game.GetMap() .. "'", function(data)
		...
		for k,v in pairs(data) do
			...
			elseif Type == SPECIAL["AREA_BONUS1"] or Type == SPECIAL["AREA_BONUS2"] then
				local CSplit = string.Explode(";", Data)
				local areaBottom = ToVector(CSplit[1]) + Vector(0, 0, 2)
				local areaTop = ToVector(CSplit[2]) + Vector(0, 0, 2)
				local AreaID = #AreaEnts["Custom"] + 1
				...
				AreaEnts["Custom"][AreaID].min = areaBottom
				AreaEnts["Custom"][AreaID].max = areaTop
				AreaEnts["Custom"][AreaID].areatype = Type
				AreaEnts["Custom"][AreaID]:Spawn()
				
				if Type == SPECIAL["AREA_BONUS1"] then
					local brefB, brefT = Vector(areaBottom.x, areaBottom.y, areaBottom.z), Vector(areaTop.x, areaTop.y, areaTop.z)
					MAP_BONUS = (brefB + brefT) / 2
				end
			...
			elseif Type == SPECIAL["SOUND_BLOCK"] then
				if not Data or Data == "" then continue end
				for k,v in pairs(ents.FindByClass(Data)) do v:Remove() end
			...
			elseif Type == SPECIAL["CUSTOM_EMAN"] then
				for k,v in pairs(ents.FindByClass("trigger_teleport")) do
					local pos = v:GetPos()
					if pos.x == -1316 and (pos.y == -10863 or pos.y == -10974 or pos.y == -10842) then
						v:SetPos(Vector(pos.x, pos.y, pos.z + 14.5))
						local Min, Max = v:GetCollisionBounds()
						Min.y, Max.y = Min.y + 64, Max.y - 64
						v:SetCollisionBounds(v:GetPos(), Min, Max)
					end
				end
			end
		end
