...
Secure.Setup = {
	...
	{ 32, "Set map tier", Admin.Level.Super, { 515, 402, 100, 25 } },
	{ 33, "Set map type", Admin.Level.Super, { 620, 402, 100, 25 } },
	...
	{ 34, "Incognito admin", Admin.Level.Super, { 515, 402, 120, 25 } },
...
function Admin:HandleButton( ply, args )
	...
	elseif ID == 32 then
		local tabRequest = Admin:GenerateRequest( "Please enter map tier. This shows the difficulty of the map (Default is 1)", "Map Tier", tostring( Timer.Tier ), ID )
		Core:Send( ply, "Admin", { "Request", tabRequest } )
	elseif ID == 33 then
...
function Admin:HandleRequest( ply, args )
	...
	elseif ID == 11 then
		...
		if nValue > 0 then
			...
			if Core:Assert( Check, "szMap" ) then
				...
				sql.Query( "INSERT INTO game_map VALUES ('" .. game.GetMap() .. "', " .. Timer.Multiplier .. ", " .. Timer.Tier .. ", " .. Timer.Type .. ", NULL, 0, " .. szValue .. ")" )
			end
	...
	elseif ID == 21 then
		...
		if Core:Assert( Check, "szMap" ) then
			...
			sql.Query( "INSERT INTO game_map VALUES ('" .. game.GetMap() .. "', " .. Timer.Multiplier .. ", " .. Timer.Tier .. ", " .. Timer.Type .. ", " .. Timer.BonusMultiplier .. ", 0, NULL)" )
		end
	...
	elseif ID == 32 then
		...
		local Check = sql.Query( "SELECT szMap FROM game_map WHERE szMap = '" .. game.GetMap() .. "'" )
		if Core:Assert( Check, "szMap" ) then
			sql.Query( "UPDATE game_map SET nTier = " .. Timer.Tier .. " WHERE szMap = '" .. game.GetMap() .. "'" )
		else
			sql.Query( "INSERT INTO game_map VALUES ('" .. game.GetMap() .. "', " .. Timer.Multiplier .. ", " .. Timer.Tier .. ", " .. Timer.Type .. ", NULL, 0, NULL)" )
		end
		
		RTV:UpdateMapListVersion()
		Core:Send( ply, "Print", { "Admin", Lang:Get( "AdminSetValue", { "Tier", Timer.Tier } ) } )
	elseif ID == 33 then
		...
		local Check = sql.Query( "SELECT szMap FROM game_map WHERE szMap = '" .. game.GetMap() .. "'" )
		if Core:Assert( Check, "szMap" ) then
			sql.Query( "UPDATE game_map SET nType = " .. Timer.Type .. " WHERE szMap = '" .. game.GetMap() .. "'" )
		else
			sql.Query( "INSERT INTO game_map VALUES ('" .. game.GetMap() .. "', " .. Timer.Multiplier .. ", " .. Timer.Tier .. ", " .. Timer.Type .. ", NULL, 0, NULL)" )
		end
		
		RTV:UpdateMapListVersion()
		Core:Send( ply, "Print", { "Admin", Lang:Get( "AdminSetValue", { "Type", Timer.Type } ) } )
	end
end