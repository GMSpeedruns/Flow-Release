	...
	if szIdentifier == "Vote" then
		...
		for i = 1, 7 do
			if i < 6 then
				...
				Cache.V_Data[ i ] = tab[ 1 ] .. " (" .. wnd.Data[ i ][ 1 ] .. " pts - Tier " .. wnd.Data[ i ][ 2 ] .. " " .. _C.MapTypes[ wnd.Data[ i ][ 3 ] ] .. ")"
			else
		...
		for i = 1, _C.PageSize do
			...
			if Item then
				if wnd.szType == "Completed" then
					Text = Index .. ". [" .. Timer:Convert( Item[ 2 ] ) .. "] " .. Item[ 1 ] .. " (" .. math.floor( Item[ 3 ] ).. " / " .. (wnd.tabData[ Item[ 1 ] ][ 1 ] and wnd.tabData[ Item[ 1 ] ][ 1 ] or "?") .. " pts)"
	...
	elseif ID == "Top" then
		if Key > 0 and Key < 8 and not KeyLimit then
			...
			if Item then
				...
				Link:Print( "Surf Timer", { GUIColor.White, Item[ 1 ] .. " is currently ranked #" .. Index .. " with " .. Item[ 2 ] .. " points, which set him to the rank of ", Rank and Rank[ 2 ] or GUIColor.White, Rank and Rank[ 1 ] or "Unknown" } )
			end
		elseif not KeyLimit and ((Key == 8 and wnd.nPage != 1) or (Key == 9 and wnd.nPage != wnd.nPages)) then
			local bPrev = Key == 8 and true or false
			wnd.nPage = wnd.nPage + (bPrev and -1 or 1)
		...
		elseif not KeyLimit and ((Key == 8 and wnd.nPage != 1) or (Key == 9 and wnd.nPage < wnd.nPages)) then
			...
			for i = 1, _C.PageSize do
				...
				if Item then
					local Text = ""
					if wnd.szType == "Completed" then
						Text = Index + i .. ". [" .. Timer:Convert( Item[ 2 ] ) .. "] " .. Item[ 1 ] .. " (" .. math.floor( Item[ 3 ] ).. " / " .. (wnd.tabData[ Item[ 1 ] ][ 1 ] and wnd.tabData[ Item[ 1 ] ][ 1 ] or "?") .. " pts)"
					elseif wnd.szType == "Left" then
