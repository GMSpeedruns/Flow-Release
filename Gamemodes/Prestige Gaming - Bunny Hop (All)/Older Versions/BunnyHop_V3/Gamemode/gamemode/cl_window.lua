...
function LoadWindowData(identifier, window, arg)
	...
	elseif identifier == "Mode" then
		MODE_NAME[6] = "Close"
		for i = 1, 6 do
			data.Labels[i] = wnd.MakeLabel{ parent = window, x = 15, y = data.Offset, font = "HUDLabelSmall", color = i == PlayerMode and Color(212, 68, 68, 255) or COLOR_WHITE, text = (i > 5 and 0 or i) .. ". " .. MODE_NAME[i] }
			data.Offset = data.Offset + (i == 5 and 24 or 16)
		end
		MODE_NAME[6] = nil
	elseif identifier == "WR" then
		data.View = arg or MODE_NORMAL
		...
		data.Labels[lID] = wnd.MakeLabel{ parent = window, x = 15, y = data.Offset, font = "HUDLabelSmall", color = COLOR_WHITE, text = "Press '9' to cycle through WRs" }
		data.Offset = data.Offset + 16
	...
	elseif identifier == "Rank" then
		...
		if arg and type(arg) == "table" then
			...
			wnd["Visible"]["Title"] = "Ranks - Points: " .. tonumber(string.format("%.2f", data.RankWeight))
		end
		...
		for i = 1, 10 do
			...
			if i > 7 then
				lblText = "(" .. string.gsub(tostring(i), "1", "") .. ") " .. navigationText[i - 7]
			elseif GAMEMODE.RankList[i] then
				lblText = i .. ". " .. GAMEMODE.RankList[i][1]
				lblColor = GAMEMODE.RankList[i][2] or COLOR_WHITE
				if i == data.RankID then lblFont = "HUDLabelSpecial" end
			end
...
function WindowThink()
	...
	elseif Data.ID == "Mode" then
		if Key > 0 and Key < 6 and not KeyLimit and not Data.Selected then
			Data.Selected = true
	...
	elseif Data.ID == "WR" then
		if Key == 9 and not KeyLimit then
			local NewView = Data.View + 1 > MODE_AUTO and MODE_NORMAL or Data.View + 1
	...
	elseif Data.ID == "Rank" then
		if Key == 8 and not KeyLimit and Data.Page != 1 then
			...
			for i = 1, 7 do
				Data.Labels[i]:SetText(Index + i .. ". " .. GAMEMODE.RankList[Index + i][1])
