...
function LoadMapTriggers()
	if not currentMapData or not currentMapData[1] then MsgN("No area positions found for map: " .. game.GetMap()) return end

	local startBottom = currentMapData[1] + Vector(0, 0, 2)
	local startTop = currentMapData[2] - Vector(0, 0, 16)
	...
	if not MAP_SPAWNPOINT and game.GetMap() == "kz_bhop_yonkoma" then
