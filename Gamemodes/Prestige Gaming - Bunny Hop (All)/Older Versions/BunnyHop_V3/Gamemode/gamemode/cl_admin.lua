...
function AdminLoad(window)
	...
	local DefaultClickLine = list.OnClickLine
	list.OnClickLine = function(parent, line, selected)
		DefaultClickLine(parent, line, selected)
		for k,v in pairs(Admin.Functions) do
...
function AdminDisplay(id, window)
	...
	elseif id == 103 then
		...
		local DefaultClickLine = data.OnClickLine
		data.OnClickLine = function(parent, line, selected)
			DefaultClickLine(parent, line, selected)
	...
	elseif id == 201 then
		...
		data.OnClickLine = function(parent, line, selected)
			...
			elseif Admin.SelectedData and type(Admin.SelectedData) == "table" then
				Admin.SelectedData = {line:GetValue(4), Admin.SelectedData[1], line:GetValue(2)}
			else
				AddText(TEXT_ADMIN, "Please select a mode first!")
			end
		end
	...
	elseif id == 203 then
		...
		table.insert(Admin.Items, wnd.MakeButton{ parent = window, w = 100, h = 30, x = sx + 10, y = sy + 45, text = "Slap / Unstuck", id = 2030, onclick = AdminExecute })
...
function AdminExecute(parent)
	...
	elseif id == 1051 then
		...
		PromptForChoice("[Special] Add: Select Type", Menu_Items, function(Dlg, Itm, Prm)
			PromptStringRequest("[Special] Add - Enter Data - " .. Admin.Strings.Specials[Itm.ID][1], "Data (Player Pos: " .. ((LocalPlayer() and LocalPlayer():GetPos()) and GetVectorString(LocalPlayer():GetPos(), true) or "") .. ")", Admin.Strings.Specials[Itm.ID][2], function(Str)
