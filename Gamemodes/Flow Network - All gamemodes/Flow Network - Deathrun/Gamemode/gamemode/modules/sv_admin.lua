...
Secure.Setup = {
	...
	{ 4, "Slay player", Admin.Level.Admin, { 600, 77, 100, 25, true } },
	...
	{ 3, "Go to next round", Admin.Level.Super, { 495, 167, 100, 25 } },
	{ 11, "Add rounds", Admin.Level.Super, { 600, 167, 100, 25 } },
	
	{ 1, "Set zone", Admin.Level.Super, { 390, 202, 100, 25 } },
	{ 2, "Cancel creation", Admin.Level.Super, { 495, 202, 100, 25 } },
	{ 10, "Remove zone", Admin.Level.Super, { 600, 202, 100, 25 } },
	{ 6, "Reload zones", Admin.Level.Super, { 705, 202, 100, 25 } },
	
	{ 9, "Set zone height", Admin.Level.Super, { 390, 237, 100, 25 } },
	{ 20, "Cancel map vote", Admin.Level.Super, { 495, 237, 100, 25 } },
	{ 31, "Request screen", Admin.Level.Super, { 600, 237, 100, 25, true } },
	
	-- Development functionality (Available row 292Y)
	-- { 17, "Misc A", Admin.Level.Developer, { 390, 292, 100, 25 } },
	-- { 18, "Misc B", Admin.Level.Developer, { 495, 292, 100, 25 } },
	-- { 28, "Misc C", Admin.Level.Developer, { 600, 292, 205, 25 } },
	
	{ 24, "Reload admins", Admin.Level.Developer, { 390, 327, 100, 25 } },
	{ 22, "Remove map", Admin.Level.Developer, { 495, 327, 100, 25 } }, -- MANAGE SAY DO IN LUA FILE
	...
	{ 32, "Incognito admin", Admin.Level.Super, { 390, 402, 100, 25 } },
}
	...
	SQL:Prepare(
		...
		if data then
			for id,item in pairs( data ) do
				ti( Secure.Notifications, { item["nTimeout"], item["szText"], nil, id } )
			end
		end
...
function Admin:CheckVIP( ply, nTime )
	-- Fix for when stupid people rejoin f*cking often when they're banned
...
function Admin:ExecuteRequest( szPath, callback, errorcall, param )
	http.Fetch( szPath, function( body ) callback( body, param ) end, errorcall or function( err ) print( "[HTTP Error] Couldn't retrieve data from (Reason: " .. err .. "): ", szPath ) end )
end
...
function Admin:AddNotification( szText )
	Core:Broadcast( "Print", { "Flow Network", szText } )
end
...
function Admin.VIPProcess( ply, args, extra )
	if not ply.IsVIP or not ply.VIPLevel then
		return Core:Send( ply, "Print", { "Flow Network", Lang:Get( "MiscVIPRequired" ) } )
	end
...
function Admin.CommandProcess( ply, args )
	...
	if #args == 0 then
		...
		elseif szID == "slay" and nAccess >= Admin.Level.Admin then
			if not args[ 2 ] then return Core:Send( ply, "Print", { "Admin", "Please enter a valid Steam ID like this: !admin " .. szID .. " STEAM_0:ID" } ) end
...
function Admin:HandleButton( ply, args )
	...
	elseif ID == 3 then
		local round = Round:GetRound()
		if round > 0 then
			local rounds = math.max( round - 1, 0 )
			Round:SetRounds( rounds )
			Round:SetRound( Round.PREPARING )
			
			Core:Send( ply, "Print", { "Admin", "Round has been changed to " .. rounds } )
		end
	elseif ID == 4 then
		...
		if IsValid( target ) then
			...
			local tabRequest = Admin:GenerateRequest( "Enter reason for slay", "Slay player", "AFK", ID )
	...
	elseif ID == 9 then
		...
		Core:Send( ply, "Admin", { "Query", tabQuery } )
	elseif ID == 10 then
		Core:Send( ply, "Print", { "Admin", "Zones can only be removed by editing the Lua file, sorry." } )
	elseif ID == 11 then
		local tabRequest = Admin:GenerateRequest( "Enter amount of rounds to be added (to remove use negative numbers)", "Remove rounds", "1", ID )
		Core:Send( ply, "Admin", { "Request", tabRequest } )
	elseif ID == 12 then
		local target = Admin:FindPlayer( Steam )
	...
	elseif ID == 22 then
		Core:Send( ply, "Print", { "Admin", "Maps can only be fully removed by deleting the Lua file." } )
	elseif ID == 23 then
		local target = Admin:FindPlayer( Steam )
...
function Admin:HandleRequest( ply, args )
	...
	elseif ID == 4 then
		...
		if IsValid( target ) then
			...
			Core:Broadcast( "Print", { "General", Lang:Get( "AdminPlayerSlay", { szName, szReason } ) } )
			Core:Send( ply, "Print", { "Admin", "You have slayed " .. szName .. " for reason: " .. szReason } )
			
			Admin:AddLog( "Admin issued slay on " .. szName .. " with reason: " .. szReason, ply:SteamID(), ply:Name() )
		else
	...
	elseif ID == 90 then
		...
		Core:Send( ply, "Print", { "Admin", "New positions for type " .. Zones.Type[ ply.ZoneData[ 1 ] ] .. ": " .. util.TypeToString( ply.ZoneData[ 2 ] ) .. " - " .. util.TypeToString( ply.ZoneData[ 3 ] ) .. " - Reload when applied" } )
	elseif ID == 11 then
		local nValue = tonumber( Value )
		if not nValue then
			return Core:Send( ply, "Print", { "Admin", Lang:Get( "AdminInvalidFormat", { Value, "Number" } ) } )
		...
		if nValue != 0 then
			local round = Round:GetRound()
			if round > 0 then
				local rounds = math.Clamp( round + nValue, 0, Round.ROUND_COUNT )
				Round:SetRounds( rounds )
				Round:SetRound( Round.PREPARING )
