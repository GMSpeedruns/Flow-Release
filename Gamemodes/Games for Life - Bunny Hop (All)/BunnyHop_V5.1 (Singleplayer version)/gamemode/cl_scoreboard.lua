...
local icon_sadmin = Material("icon16/shield_add.png")
local icon_admin = Material("icon16/shield.png")
local icon_mod = Material("icon16/report_user.png")
	...
	function but:Paint(w, h)
		...
		if IsValid(ply) && ply:IsPlayer() then
			...
			if ply:IsUserGroup( "superadmin" ) then
				surface.SetMaterial( icon_sadmin )
				surface.SetDrawColor( color_white )
				surface.DrawTexturedRect( s + 4, h / 2 - 8, 16, 16 )
				s = s + 20
			elseif ply:IsAdmin() then
				surface.SetMaterial( icon_admin )
				surface.SetDrawColor( color_white )
				surface.DrawTexturedRect( s + 4, h / 2 - 8, 16, 16 )
				s = s + 20
			elseif ply:IsUserGroup( "mod" ) then
				surface.SetMaterial( icon_mod )
				surface.SetDrawColor( color_white )
				surface.DrawTexturedRect( s + 4, h / 2 - 8, 16, 16 )
				s = s + 20
			end
...
function GM:ScoreboardShow()
	if IsValid(menu) then
		...
		local name = Label("GFL Bunny Hop", menu.Credits)
		name:Dock(LEFT)
		...
		cred.DoClick = function()
			gui.OpenURL( "http://gflclan.com/topic/751-bhop-server-information/" )
			--gui.OpenURL( "http://steamcommunity.com/id/Gravious_/" ) -- This is my private profile if anyone wishes for it.
		end
		...
		local mw,mh = surface.GetTextSize("Decently Wide")
		...
		if not ply:IsBot() then
			...
			function chatmute:DoClick()
				if IsValid(ply) then
					...
					Message:Print( Config.Prefix.Game, "PlayerChatMute", { ply:Name(), ply.ChatMuted and "chat muted" or "chat unmuted" } )
				end
			end
