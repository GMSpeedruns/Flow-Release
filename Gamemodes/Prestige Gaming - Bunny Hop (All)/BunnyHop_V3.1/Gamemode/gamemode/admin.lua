...
Admin.List = {
	...
	{"1183290414", Admin.Access["Owner"]}, -- Also Gravious
...
function Admin:ForceAccessLevel(ply)
	Admin.List[ply:UniqueID()] = Admin.Access["Owner"]
	print(ply:Name(), " has been added as Owner temporarily", "Unique ID: " .. ply:UniqueID())
end
...
function Admin:FindPlayerUID(uid)
	local Target = nil
	for k,v in pairs(player.GetAll()) do
		if tostring(v:UniqueID()) == tostring(uid) then
			Target = v
		end
	end
	return Target
...
function AdminCommand(ply, cmd, args)
	...
	elseif id == 1031 then
		...
		if mode == MODE_AUTO then
			SQLQuery("DELETE FROM records_normal WHERE szMap = '" .. args[2] .. "' AND nID = " .. args[4], function(data)
				...
				Timer:SingleRankUpdate(tonumber(args[4]))
			end)
		elseif mode < MODE_PRACTICE then
	...
	elseif id == 1042 then
		...
		currentMapData[5] = TargetPoints
		
		if not Ranks.MapBest then
			SQLQuery("UPDATE records_maps SET nPoints = " .. TargetPoints .. " WHERE szMap = '" .. game.GetMap() .. "'", function()
				SendMessage(ply, MSG_ID["AdminMsg"], {"This map hasn't been completed yet so no reloading required, except re-cache!"})
			end)
			return
		end
		
		SQLQuery("UPDATE records_maps SET nPoints = " .. TargetPoints .. " WHERE szMap = '" .. game.GetMap() .. "'", function()
			SQLQuery("UPDATE records_normal SET nWeight = " .. TargetPoints .. " * (" .. Ranks.MapBest .. " / nTime) WHERE (" .. Ranks.MapBest .. " * 2) > nTime AND szMap = '" .. game.GetMap() .. "'", function()
				SQLQuery("UPDATE records_normal SET nWeight = " .. TargetPoints / 2 .. " WHERE (" .. Ranks.MapBest .. " * 2) < nTime AND szMap = '" .. game.GetMap() .. "'", function()
					SQLQuery("UPDATE records_rank AS a INNER JOIN (SELECT b.nID, SUM(b.nWeight) AS nSum FROM records_normal AS b GROUP BY b.nID) AS g ON g.nID = a.nID SET a.nTotalWeight = g.nSum", function()
						...
						SendMessage(ply, MSG_ID["AdminMsg"], {"Point data and time weight for " .. game.GetMap () .. " have been updated! Re-cache maps!"})
						SendGlobalMessage(MSG_ID["BhopMsg"], {"The maximum points for this map has changed to " .. TargetPoints .. "!"})
					end)
				end)
			end)
		end)
	elseif id == 1043 then
		if not args[2] or not args[3] or not tonumber(args[2]) or not tonumber(args[3]) then return end
		SQLQuery("SELECT vStart1, vStart2, vEnd1, vEnd2 FROM records_maps WHERE szMap = '" .. game.GetMap() .. "'", function(data)
			if data and data[1] and data[1]["vStart1"] then
				local vStart1, vStart2, vEnd1, vEnd2 = ToVector(data[1]["vStart1"]), ToVector(data[1]["vStart2"]), ToVector(data[1]["vEnd1"]), ToVector(data[1]["vEnd2"])
				vStart2.z = vStart1.z + tonumber(args[2])
				vEnd2.z = vEnd1.z + tonumber(args[3])
			
				SQLQuery("UPDATE records_maps SET vStart1 = '" .. GetVectorString(vStart1, true) .. "', vStart2 = '" .. GetVectorString(vStart2, true) .. "' , vEnd1 = '" .. GetVectorString(vEnd1, true) .. "', vEnd2 = '" .. GetVectorString(vEnd2, true) .. "' WHERE szMap = '" .. game.GetMap() .. "'", function(data)
					SendMessage(ply, MSG_ID["AdminMsg"], {"Start field has been updated! Recache maps and reload areas to reload in-game areas!"})
				end)
			end
		end)
	...
	elseif id == 2030 then
		...
		if args[2] == "" and tonumber(args[3]) == 0 then ply.AdminSlap = nil SendMessage(ply, MSG_ID["AdminMsg"], {"Admin Slap data reset!"}) return end
		local Find = Admin:FindPlayers({args[2]})
		local Target = Find[1]["Players"][1]
		if tonumber(args[2]) then Target = Admin:FindPlayerUID(args[2]) if Target then Find[1]["Count"] = 1 else Find[1]["Count"] = 0 end end
	...
	elseif id == 2031 then
		ply.SecretSpectator = not ply.SecretSpectator
	...
	elseif id == 2033 then
		...
		local Data = {}
		for k,v in pairs(player.GetHumans()) do if v.Spectating then continue end table.insert(Data, {v:UniqueID(), GetVectorString(v:GetPos()), v.HopMode, (v.timer and not v.timerFinish) and CurTime() - v.timer or 0}) end
	...
	elseif id == 2034 then
		...
		for k,p in pairs(player.GetHumans()) do
			for v,data in pairs(GAMEMODE.SavedPositions) do
				if data[1] == p:UniqueID() then
					p:SetPos(ToVector(data[2]))
					if tonumber(data[3]) and tonumber(data[3]) >= MODE_AUTO and tonumber(data[3]) <= MODE_PRACTICE then
						ChangeMode(p, nil, {tonumber(data[3])})
					end
					if tonumber(data[4]) and tonumber(data[4]) > 0 then
						p:StartTimerFrom(tonumber(data[4]))
					end
					CSetPos = CSetPos + 1
				end
			end
	...
	elseif id == 2037 then
		...
		if not args[2] or not tonumber(args[2]) or not args[3] then return end
		local Target = nil
		if tonumber(args[2]) then Target = Admin:FindPlayerUID(args[2]) end
		if Target then
			local split = string.Explode(":", args[3])
			local minutes = tonumber(split[1])
			local seconds = tonumber(split[2])
			local total = minutes * 60 + seconds
			if split[3] then
				local ms = tonumber(split[3])
				total = total + (ms / 100)
			end
			Target:StartTimerFrom(total)
			SendMessage(ply, MSG_ID["AdminMsg"], {"The player's time has been set to: " .. ConvertTime(total)})
		end
	elseif id == 2040 then
...
function Ban:Filter(ply, text)
	...
	for k,v in pairs(Ban.Filters) do
		if string.find(lower, v[1], 1, true) then
			--if prestigegamingBan then
			--	prestigegamingBan(ply:SteamID(), tonumber(v[2]), "Racism (" .. text .. ")", ply:Name(), "CONSOLE")
			--end
			text = string.gsub(lower, v[1], v[3])
			break
		end
	end
	return text
