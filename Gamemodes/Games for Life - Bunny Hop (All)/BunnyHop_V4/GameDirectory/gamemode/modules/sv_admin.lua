...
function Admin:LoadAdmins()
	...
	for _, data in pairs( Admins ) do
		...
		table.insert( Admin.List, { tab.UID, Admin.Access[ tab.Access ] } )
	end
	
	print( #Admin.List .. " admins loaded!" )
end

function Admin:AddAdmin( ply, nAccess )
	table.insert( Admin.List, { ply:UniqueID(), nAccess } )
	
	-- To-Do: Rewrite admin list here
end
