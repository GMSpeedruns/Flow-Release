...
function RTV:StartVote()
	...
	for map, voters in pairs( RTV.Nominations ) do
		local nCount = #voters
		if not TempList[ nCount ] then
			TempList[ nCount ] = { map }
		else
			table.insert( TempList[ nCount ], map )
		end
	end
	...
	for i = 64, 1, -1 do
		if TempList[ i ] then
			for _, map in pairs( TempList[ i ] ) do
...
function RTV:EndVote()
	...
	elseif nWin == 6 then
		...
		for _, p in pairs( player.GetHumans() ) do
			...
			p:SendLua( "Timer:SetRemaining(" .. CurTime() .. "," .. (RTV.DefaultExtend * 60) .. ")" )
		end
...
function RTV:CheckLatest()
	...
	local Split = string.Explode( FS.Delimiter, szLatest )
	if #Split != 3 then return end
	RTV.LatestList = Split
	table.insert( Split, 1, game.GetMap() )
	table.remove( Split, 4 )
	file.Write( FS.Folders.Log .. "latestmaps.txt", string.Implode( FS.Delimiter, Split ) )
end