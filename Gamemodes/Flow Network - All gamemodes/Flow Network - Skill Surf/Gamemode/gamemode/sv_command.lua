...
function Command:Init()
	...
	-- Windows
	self:Register( { "style", "mode", "surf", "styles", "modes" }, function( ply )
		Command:RemoveLimit( ply )
	...
	self:Register( { "mywr", "mywrs", "wr1", "wr#1", "wrcount", "wrcounter", "countwr", "wramount" }, function( ply )
		if not WRData then
			WRData = sql.Query( "SELECT * FROM game_times AS t1 WHERE nTime = (SELECT MIN(nTime) FROM game_times AS t2 WHERE t2.szMap = t1.szMap)" )
		end
		
		if not WRData then
			Core:Send( ply, "Print", { "General", "There are no #1 times." } )
		else
			local tab = {}
			for _,d in pairs( WRData ) do
				if d["szUID"] == ply:SteamID() then
	...
	self:Register( { "map", "points", "mapdata", "mapinfo", "difficulty", "tier" }, function( ply, args )
		if #args > 0 then
			...
			if RTV:MapExists( args[ 1 ] ) then
				...
				Core:Send( ply, "Print", { "General", Lang:Get( "MapInfo", { data[ 1 ], data[ 2 ] or 1, "No more details", "", Timer.Tier or 1, _C.MapTypes[ Timer.Type or 0 ] } ) } )
			else
			...
			Core:Send( ply, "Print", { "General", Lang:Get( "MapInfo", { game.GetMap(), Timer.Multiplier or 1, szPoints, szBonus, Timer.Tier or 1, _C.MapTypes[ Timer.Type or 0 ] } ) } )
		end
	end )
	...
	self:Register( { "end", "goend", "gotoend", "tpend" }, function( ply )
		if ply.Style == _C.Style.Practice then
			...
			if vPoint then
				ply:SetPos( vPoint )
				Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "PlayerTeleport", { "the normal end zone!" } ) } )
			else
				Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "MiscZoneNotFound", { "normal end" } ) } )
			end
		else
			Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "StyleTeleport" ) } )
		end
	end )
	...
	self:Register( { "endbonus", "endb", "bend", "gotobonus", "tpbonus" }, function( ply )
		if ply.Style == _C.Style.Practice then
			...
			if vPoint then
				ply:SetPos( vPoint )
				Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "PlayerTeleport", { "the bonus end zone!" } ) } )
			else
				Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "MiscZoneNotFound", { "bonus end" } ) } )
			end
		else
			Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "StyleTeleport" ) } )
		end
...
function Command.Restart( ply )
	...
	if ply:Team() != _C.Team.Spectator then
		...
		Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "SpectateRestart" ) } )
	end
end
...
function Command.Style( ply, _, varArgs )
	...
	if tonumber( varArgs[ 1 ] ) == ply.Style then
		if ply.Style == _C.Style.Bonus then
			...
			return Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "StyleEqual", { Core:StyleName( ply.Style ) } ) } )
		end
	end
	
	local nStyle = tonumber( varArgs[ 1 ] ) or _C.Style.Normal
	if nStyle == _C.Style.Bonus and not Zones.BonusPoint then
		return Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "StyleBonusNone" ) } )
