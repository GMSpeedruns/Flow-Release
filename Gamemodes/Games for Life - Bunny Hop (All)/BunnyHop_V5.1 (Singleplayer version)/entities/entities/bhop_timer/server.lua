...
function ENT:StartTouch( ent )  
	if IsValid( ent ) and ent:IsPlayer() and ent:Team() != TEAM_SPECTATOR then
		...
		elseif self:GetAreaType() == Config.Area.Freestyle then
			ent:StartFreestyle()
		elseif self:GetAreaType() == Config.Area.Function and self.AtStart then
			self.SetFunction( ent )
		end
	end
end
...
function ENT:EndTouch( ent )
	if IsValid( ent ) and ent:IsPlayer() and ent:Team() != TEAM_SPECTATOR then
		if self:GetAreaType() == Config.Area.Start then
			ent:SetInSpawn( false, false )
			ent:StartTimer()
		elseif self:GetAreaType() == Config.Area.BonusA then
			ent:SetInSpawn( false, true )
			ent:BonusStart()
		elseif self:GetAreaType() == Config.Area.StepSize and self.steps then
		...
		elseif self:GetAreaType() == Config.Area.Freestyle then
			ent:StopFreestyle()
		elseif self:GetAreaType() == Config.Area.Function and self.AtEnd then
			self.SetFunction( ent )
		end
	end
end