...
Player.LadderScalar = 1.50 --1.50 -- On 7.12 was 1.50 ~ Noob got Nightmare -> On 7.14 back to 1.5
...
function Player:LoadStyle( ply, nStyle )
	...
	Core:Send( ply, "Print", { "Surf Timer", Lang:Get( "StyleChange", { Core:StyleName( ply.Style ) } ) } )
end
...
function Player:GetMatchingStyles( nStyle )
	local tab = { _C.Style.Normal, _C.Style.Bonus }
	
	if nStyle == _C.Style.SW or nStyle == _C.Style.HSW then
		tab = { _C.Style.SW, _C.Style.HSW }
	end
...
function Player:SendRemoteWRList( ply, szMap, nStyle, nPage, bUpdate )
	...
	if bZero or SendCount == 0 then
		...
		Core:Send( ply, "Print", { "Surf Timer", "No WR data found for " .. szMap .. " on style " .. Core:StyleName( nStyle ) } )
	else
		if bUpdate then
