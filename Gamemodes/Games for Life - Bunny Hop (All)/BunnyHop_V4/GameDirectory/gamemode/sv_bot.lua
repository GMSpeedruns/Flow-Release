...
function Bot:CreateHook()
	if not Bot.Hooked then
		hook.Add( "SetupMove", "BotRecording", Bot.MoveBotRecord )
		Bot.Hooked = true
	end
end
...
function Bot:Setup()
	for mode, enabled in pairs( Bot.Modes ) do
		...
		Bot.Replayers[ mode ] = nil
		Bot.ReplayData[ mode ] =  { Frame = 1, Frames = 0, Start = nil, Position =  {}, Angle = {}, Saved = true }
		Bot.Data[ mode ] = nil
	end
...
function Bot.MoveBotRecord( ply, data )
