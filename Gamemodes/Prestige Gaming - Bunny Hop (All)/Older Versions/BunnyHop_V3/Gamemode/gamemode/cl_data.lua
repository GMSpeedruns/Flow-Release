...
TopList = {}
MapsBeatList = {}
RecordList = {[MODE_NORMAL] = {}, [MODE_SIDEWAYS] = {}, [MODE_WONLY] = {}, [MODE_AUTO] = {}}
...
local msgData = {[TEXT_BHOP] = {"Bhop", Color(168, 223, 133)}, [TEXT_TIMER] = {"Timer", Color(133, 156, 223)}, [TEXT_ADMIN] = {"Admin", Color(223, 133, 133)}, [TEXT_BOT] = {"Bot", Color(133, 201, 223)}}
...
function ParseData(ID, DATA)
	...
	elseif ID == DATA_ID["WRFull"] then
		RecordList = DATA
		if IsWindowActive("WR") then SetActiveWindow("WR") end
...
function ProcessView(data)
	if data[1] then
		if not table.HasValue(Spectator.List, data[2]) then
			table.insert(Spectator.List, data[2])
		end
	else
		local ID = 0
		for k,v in pairs(Spectator.List) do
			if v == data[2] then ID = k break end
		end
		if ID > 0 then table.remove(Spectator.List, ID) end
	end
end
...
function ProcessTime(data)
	if not data[1] then
		...
		Spectator.Data["BestTime"] = data[3] and ConvertTime(tonumber(data[3])) or nil
		...
		Spectator.Data["StartTime"] = data[2] and tonumber(data[2]) + Spectator.TimeDifference or nil
		Spectator.Data["BestTime"] = data[4] and " / " .. ConvertTime(tonumber(data[4])) or ""
		Spectator.Data["Contains"] = true
	end
end
