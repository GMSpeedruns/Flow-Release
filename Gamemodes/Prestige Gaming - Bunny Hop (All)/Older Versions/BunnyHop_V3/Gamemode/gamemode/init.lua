...
function Spectator:Notify(target, ply, leave)
	if ply.SecretSpectator then return end
	if leave then
		SendData(target, DATA_ID["SpecView"], {false, ply:Name()})
		return
	else
		SendData(target, DATA_ID["SpecView"], {true, ply:Name()})
	end
	
	if target.InSpawn or not target.timer then
		SendData(ply, DATA_ID["SpecTime"], {false, false, (target.CurrentRecord and target.CurrentRecord > 0) and target.CurrentRecord or nil, CurTime()})
	elseif not target.InSpawn and target.timer and target.timer > 0 then
		SendData(ply, DATA_ID["SpecTime"], {false, target.timer, (target.CurrentRecord and target.CurrentRecord > 0) and target.CurrentRecord or nil, CurTime()})
	end
end

function Spectator:NotifyBot(ply)
	if not Bot.Data then return end
	SendData(ply, DATA_ID["SpecTime"], {true, tonumber(Bot.PlaybackStart), Bot.Data["Name"], tonumber(Bot.Data["Time"]), CurTime()})
end
...
function ConvertTime(time)
	if time > 3600 then
		return string.format("%d:%.2d:%.2d:%.2d", math.floor(time / 3600), math.floor(time / 60 % 60), math.floor(time % 60), math.floor(time * 60 % 60))
	else
